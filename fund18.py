...
def rsiFunc(prices, n=14):
	...
	for i in range(n, len(prices)):
		...
		rs = up/down #do we need this again? it's already above
		rsi[i] = 100. - 100./(1.+rs)
...
def computeMACD(x, slow=26, fast=12):
	"""
	Compute the MACD (Moving Average Convergence/Divergence) using a fast and slow exponential moving average
...
def graphData(stock, MA1, MA2):
	...
	try:
		...
		#print str(datetime.datetime.fronttimestamp(int(time.time())).strftime('%Y-%m-%d %H:%M:%S'))
		print str(datetime.datetime.fromtimestamp(int(time.time())).strftime('%Y-%m-%d %H:%M:%S'))
		#this is close high low open
		...
		if float(pbr) <1:
			...
			if 0 < float(PEG5) < 2:
				...
				if float(PE12) < 15:
					...
					print "PEG foward 5 years: ", PEG5
					print "Trailing PE (12 months): ", PE12
					print "Debt to equity: ", DE
					print "-" * 50
